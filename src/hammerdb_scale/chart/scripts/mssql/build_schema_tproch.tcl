...
foreach var {USERNAME PASSWORD SQL_SERVER_HOST TPROCH_DATABASE_NAME TPROCH_SCALE_FACTOR TPROCH_DRIVER TPROCH_BUILD_THREADS TPROCH_BUILD_VIRTUAL_USERS} {
...
# SSL/TLS connection settings with proper boolean conversion
if {[info exists ::env(MSSQLS_ENCRYPT_CONNECTION)]} {
    if {$::env(MSSQLS_ENCRYPT_CONNECTION) eq "true"} {
...
if {[info exists ::env(MSSQLS_TRUST_SERVER_CERT)]} {
    if {$::env(MSSQLS_TRUST_SERVER_CERT) eq "true"} {
...
# ODBC driver setting
if {[info exists ::env(MSSQLS_ODBC_DRIVER)]} {
    diset connection mssqls_linux_odbc $::env(MSSQLS_ODBC_DRIVER)
} else {
